...
<body>
    ...
    <h1 style="text-align:center;">ORDEN DE SERVICIO N° &nbsp; {{$num_id}}</h1>
    ...
    <table style="font-size:8px;" border="1">
        ...
        <?php foreach ($elements as $item) { ?>
            <tr style="text-align:center">
                <td colspan="4" style="height:310px; font-weight:bold;">{{$item["codigo"]}}</td>
    ...
    <table style="font-size:6px; border: 1px solid black;" border="1">
        ...
        <tr>
            <td colspan="3" rowspan="2">{{ strtoupper(Auth::user()->paterno) }} {{ strtoupper (Auth::user()->materno) }} {{ strtoupper(Auth::user()->name) }}</td>
